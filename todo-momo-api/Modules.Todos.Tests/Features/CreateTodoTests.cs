...
namespace Modules.Todos.Tests.Features;
...
public class CreateTodoTests
{
    ...
    public async Task Handle_WithValidCommand_PersistsTodoAndReturnsSuccessWithId()
    {
        ...
        await Assert.That(result.Value.Id).IsNotEqualTo(Guid.Empty);
        await Assert.That(result.Value.Description).IsEqualTo("Buy milk");
        await Assert.That(result.Value.DueDate).IsEqualTo(command.DueDate);
        await Assert.That(result.Value.IsCompleted).IsFalse();

        var stored = await context.TodoItems.SingleAsync();
        await Assert.That(stored.Id).IsEqualTo(result.Value.Id);
